...
def interpolate_fields_fine_coarse_rbf(fine_field_dir,
                                          coarse_field_dir,
                                          output_dir,
                                          fine_case_prefix_and_name,
                                          coarse_case_prefix_and_name,
    ...
    for field_name in fields_list:
        print('    Interpolating '+field_name + ' using nearest to start')
        field_fine = np.load(os.path.join(fine_field_dir,fine_case_prefix_and_name+'_'+field_name+'.npy'))
        ...
        interp_field = griddata((x_fine,y_fine,z_fine),
                                ...
                                (x_coarse,y_coarse,z_coarse),
                                method='nearest')
        print('    Smoothing  '+field_name + ' using RBF')

        smooth_field = RBFInterpolant(np.column_stack((x_coarse,y_coarse,z_coarse)), interp_field, neighbors=neighbors,sigma=sigma, eps=eps, phi=phi)(np.column_stack((x_coarse,y_coarse,z_coarse)))
        #interp_field = interpolator(np.column_stack([x_coarse,y_coarse,z_coarse],-1))
        #interp_field = griddata((x_fine,y_fine,z_fine),
        #                        field_fine,
        #                        (x_coarse,y_coarse,z_coarse),
        #                        method=interp_method)
        ...
        if len(ind_nan) >0:
            print('    Interpolation found '+ str(len(ind_nan))+' nans')
        """
        if interp_method != 'nearest':
            ind_nan = np.argwhere(np.isnan(interp_field))
            if len(ind_nan) >0:
        ...
        """
        print('    Saving interpolated field to ' + str(os.path.join(output_dir,output_case_prefix_and_name+'_'+field_name+'.npy')))
        np.save(os.path.join(output_dir,output_case_prefix_and_name+'_'+field_name+'.npy'),smooth_field)   